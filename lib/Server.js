...
const {ERRORS, EXPOSED_HEADERS, REQUEST_METHODS, TUS_RESUMABLE} = require(
    './constants');
...
class TusServer extends EventEmitter {
  ...
  handle(req, res) {
    ...
    if (req.method !== 'OPTIONS' && req.headers['tus-resumable'] ===
        undefined) {
    ...
    for (const header_name in req.headers) {
      ...
      if (header_name.toLowerCase() === 'content-type' && req.method !==
          'PATCH') {
        continue;
      }
      if (RequestValidator.isInvalidHeader(header_name,
