...
class PostHandler extends BaseHandler {
  ...
  constructor(store, options) {
    if (options.namingFunction && typeof options.namingFunction !==
        'function') {
  ...
  async send(req, res) {
    if ('upload-concat' in req.headers &&
        !this.store.hasExtension('concatentation')) {
    ...
    const file = new File(file_id, upload_length, upload_defer_length,
        upload_metadata);
    ...
    if (!RequestValidator.isInvalidHeader('content-type',
      ...
      if (new_offset === parseInt(upload_length, 10)) {
        this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, {
          file: new File(file_id, file.upload_length, file.upload_defer_length,
