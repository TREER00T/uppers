...
class PatchHandler extends BaseHandler {
    ...
    async send(req, res) {
        ...
        if (file.size !== offset) {
            // If the offsets do not match, the Server MUST respond with the 409 Conflict statuses without modifying the upload resource.
            console.log(`[PatchHandler] send: Incorrect offset - ${offset} sent but file is ${file.size}`);
        ...
        if (new_offset === parseInt(file.upload_length, 10)) {
            this.emit(EVENTS.EVENT_UPLOAD_COMPLETE, {file: new File(file_id, file.upload_length, file.upload_defer_length, file.upload_metadata), req});
        }
