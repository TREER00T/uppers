...
class BaseHandler extends EventEmitter {
  ...
  generateUrl(req, file_id) {
    return this.options.relativeLocation ? `${req.baseUrl ||
    ''}${this.options.path}/${file_id}` : `//${req.headers.host}${req.baseUrl ||
    ''}${this.options.path}/${file_id}`;
  }
  ...
  getFileIdFromRequest(req) {
    const re = new RegExp(
        `${req.baseUrl || ''}${this.options.path}\\/(\\S+)\\/?`); // eslint-disable-line prefer-template
