    ...
    create(file) {
        return new Promise((resolve, reject) => {
            return fs.open(path.join(this.directory, file.id), 'w', async (err, fd) => {
                if (err) {
                    console.log('[FileStore] create: Error', err);
                    return reject(err);
                }
                ...
                return fs.close(fd, (exception) => {
                    if (exception) {
                        console.log('[FileStore] create: Error', exception);
    ...
    write(readable, file_id, offset) {
        ...
        return new Promise((resolve, reject) => {
            stream.pipeline(readable, transform, writeable, (err) => {
                ...
                console.log(`[FileStore] write: ${bytes_received} bytes written to ${path}`);
        ...
        return new Promise((resolve, reject) => {
            ...
            fs.stat(file_path, (error, stats) => {
                if (error && error.code === FILE_DOESNT_EXIST && config) {
                    console.log(`[FileStore] getOffset: No file found at ${file_path} but db record exists`, config);
                    return reject(ERRORS.FILE_NO_LONGER_EXISTS);
                }

                if (error && error.code === FILE_DOESNT_EXIST) {
                    console.log(`[FileStore] getOffset: No file found at ${file_path}`);
                ...
                if (stats.isDirectory()) {
                    console.log(`[FileStore] getOffset: ${file_path} is a directory`);
                    return reject(ERRORS.FILE_NOT_FOUND);
                }
