...
class FileStreamSplitter extends stream.Writable {
  ...
  _write(chunk, encoding, callback) {
    Promise.resolve().then(() => {
      ...
      return this._writeChunk(chunk).then(() => callback()).catch(callback);
    }).catch(callback);
  }
  ...
  _final(callback) {
    if (this.fileDescriptor === null) {
      ...
      this._finishChunk().then(() => callback()).catch(callback);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      fs.close(this.fileDescriptor, (err) => {
        ...
        this.emit('chunkFinished',
            {path: this.currentChunkPath, size: this.currentChunkSize});
  ...
  _newChunk() {
    return new Promise((resolve, reject) => {
      this.currentChunkPath = path.join(this.directory,
